...
def test_home(request):
    ...
    if request.method == 'POST':
        ...
        if get_test_form.is_valid():
            ...
            test_exists = get_object_or_404(TmtTest, test_code=test_code)
            context['test'] = test_exists
            return redirect('/tmt/test-start/{0}'.format(test_code))
        else:
            return redirect('/tmt')
    else:
        context['get_test_form'] = GetTestForm
        template = "tmt/test_home.html"
...
def test_start(request, test_code):
    ...
    if request.is_ajax:
        if request.method == 'POST':
            ...
            test = get_object_or_404(TmtTest, test_code=test_code)
            print(json_data)
        ...
        if test.completed:
            ...
            context['test'] = test
    return render(request, 'tmt/index.html', context)
...
def send_test_data(request, test_code):
    if request.is_ajax():
            ...
            for trial in test_json['trials']:
                ...
                for card in trial['finishedCards']:
                    ...
                    for target_dict in card['clicked_targets']:
                        ...
                        current_target = Point(card=current_card, x=target['x'], y=target['y'], is_goal=target['is_target'], click_time=datetime.timedelta(hours=int(time[0]), minutes=int(time[1]), seconds=int(time[2]), milliseconds=int(time[3])*100))
                        current_target.save()
            '''
            test.completed = True
        ...
        context = {}
        return render(request, 'tmt/finished.html', context)
